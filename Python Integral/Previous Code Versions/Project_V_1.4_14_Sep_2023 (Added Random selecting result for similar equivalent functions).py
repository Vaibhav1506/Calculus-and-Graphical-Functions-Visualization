...
# Indefinite integration
    ...
    if ch2 == 1:  # trigonometric
        ...
        if ch3 == 1:
            ...
            I = random.choice([f"((log|sec{exp} + tan{exp}|)/({a})) + C", f"((log|cosec({exp}) - cot({exp})|)/({a})) + C", f"((1/(2{a}))log(|(cos({exp}) - 1)/(cos({exp}) + 1)|)) + C", f"(log|tan({exp}/2)|)/({a}) + C"])
        elif ch3 == 6:
            I = random.choice([f"(log|tan({exp}) + sec({exp})|)/({a}) + C", f"((1/(2{a}))log(|(1 + sin({exp}))/(1 - sin({exp}))|)) + C", f"((1/({a}))(log(|tan(({exp}/2) + (pi/4))|))) + C", f"(cosh{inv}(sec{exp})) + C", f"(sinh{inv}(tan{exp})) + C", f"(tanh{inv}(sin{exp})) + C"])
        elif ch3 == 7:
                    I = f"(sec({exp}))/({a}) + C" 
        elif ch3 == 8:
                    I = f"(-cosec({exp})|)/({a}) + C" 
