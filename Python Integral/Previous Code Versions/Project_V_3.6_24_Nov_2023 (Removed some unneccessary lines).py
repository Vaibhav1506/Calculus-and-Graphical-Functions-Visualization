import matplotlib.pyplot as plt, numpy as np, math, mysql.connector as mc, os
...
def create_SQL_database(host_name, user_name, password):
    try:
        ...
        co.execute(
            "CREATE TABLE IF NOT EXISTS Calculus_User_History (`Operation Type` VARCHAR(100) ,`Operation Type Choice` VARCHAR(100) ,`Function Type` VARCHAR(100) ,`Function Type Choice` VARCHAR(100) ,`Function Expression` VARCHAR(100) , `Function Expression Choice` VARCHAR(100) , `Answer` VARCHAR(100) UNIQUE);")
        obj.commit()
...
print("-" * 80)
print("1. While taking input use parentheses to avoid operator precedence confusion.")
...
print("3. Only general linear and implicit functions are present in this program so as to avoid complications.")
print("4. For entering trignometric functions use 'csc' for cosecant function.")
print("5. For exponentials use ^ sign.")
print("6. GIF or Floor function should be represented within ⌊ ⌋ and SIF or ceiling function should be represented within ⌈ ⌉")
print("7. While entering implicit functions make sure they are closed within (). For example for |sin|x|| we have to enter [sin([x])].")
print("8. Polynomial Functions can be evaluated in both the choices given.")
print("9. The First one provides a better visualization of naïve or simple functions and their roots.")
print("10. If a power's coefficient is missing or not defined then enter 0 while entering the coefficients.")
print("11. The second one provides customization and using other functions implicitly in the polynomial ones.")
print("12. While defining product between two functions or a constant and a function use * asterisk marks.")
print("13. For Polynomial Functions where the roots of x are complex numbers it's recommended not to use it in the first choice.")

n = input("Make sure you have read the above mentioned rules. Press Y to proceed (Y/N): ")

while n.upper() == "Y":
    print(
        "Required for Database connection: (You may need to try one more time again if the first time does not works.)")
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                ch1, ch2, ch3, func, func_1, func_type, inv, I, LHS = "-", "-", "-", "", "", "", '{}{}'.format(
                    get_sup('-'), get_sup('1')), "", ""
                ...
                if ch1 == 1:
                    ...
                    if ch2 == 1:
                        ...
                        expressions_list = [r"\sin^{-1}{(ax+c)}", r"\cos^{-1}{(ax+c)}", r"\tan^{-1}{(ax+c)}",
                                            r"\cot^{-1}{(ax+c)}", r"\csc^{-1}{(ax+c)}", r"\sec^{-1}{(ax+c)}"]
                        expression_list_2 = [f"sin{inv}(ax+c)", f"cos{inv}(ax+c)", f"tan{inv}(ax+c)", f"cot{inv}(ax+c)",
                                             f"csc{inv}(ax+c)", f"sec{inv}(ax+c)"]
                        new_expressions_list = [i.replace("ax+c", exp) for i in expressions_list]
                        new_expressions_list_2 = [i.replace("ax+c", exp) for i in expression_list_2]
                        ...
                        if check_duplicates(func, host_name, user_name, passwd)[0] == 1:
                            ...
                            print("Record added to database.")
                            I = f'{a}(e' + '{}{}{}{}{}{}'.format(get_sup('('), get_sup(str(a)), get_sup('x'),
                                                                 get_sup('+'), get_sup(str(c)), get_sup(')')) + ")"
                    ...
                    if ch2 == 1:
                        ...
                        expressions_list = [r"\sin{(ax+c)}", r"\cos{(ax+c)}", r"\tan{(ax+c)}", r"\cot{(ax+c)}",
                                            ...
                                            r"\csc{(ax+c)}\cot{(ax+c)}", r"\csc^{2}{(ax+c)}", r"\sec^{2}{(ax+c)}"]
                        expression_list_2 = ["sin(ax+c)", "cos(ax+c)", "tan(ax+c)", "cot(ax+c)", "csc(ax+c)",
                                             "sec(ax+c)", "sec(ax+c)tan(ax+c)", "csc(ax+c)cot(ax+c)",
                                             f"csc{get_sup('2')}(ax+c)", f"sec{get_sup('2')}(ax+c)"]
                        new_expressions_list = [i.replace("ax+c", exp) for i in expressions_list]
                        new_expressions_list_2 = [i.replace("ax+c", exp) for i in expression_list_2]
                        ...
                        expressions_list = [r"\dfrac{1}{\sqrt{1 - (ax + c)^{2}}}",
                                            r"\dfrac{-1}{\sqrt{1 - (ax + c)^{2}}}", r"\dfrac{1}{b + (ax + c)^{2}}",
                                            r"\dfrac{-1}{b + (ax + c)^{2}}",
                    ...
                    if ch2 == 1:
                        ...
                        if ch3 == 1:
                            ...
                            if check_duplicates(func_1, host_name, user_name, passwd)[0] == 1:
                                ...
                                I = (math.atan(math.sqrt(b) / (exp_upr)) / (a * math.sqrt(b))) - (
                                            math.atan(math.sqrt(b) / (exp_lwr)) / (a * math.sqrt(b)))
                        ...
                        if ch3 == 1:
                            ...
                            if check_duplicates(func_1, host_name, user_name, passwd)[0] == 1:
                                ...
                                print("Record added to database.")
                                I = (((exp_upr) * (math.log(exp_upr) - 1)) / a) - (
                                            ((exp_lwr) * (math.log(exp_lwr) - 1)) / a)
                ...
                if ch1 in range(1, 6):
                    ...
                    if I != "":
                        ...
                        print(
                            "No Mathematical Expression has been evaluated since user has not entered any expression.")
